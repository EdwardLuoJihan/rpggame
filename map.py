...
def generate_html_tree(locations, nodes, current):
    """Generates a tree of nodes in SVG format"""
    ...
    max_level = max(
        len([n for n in nodes if n[0] == i]) for i in range(1, len(locations) + 1)
    )
    ...
    for location_id, location_data in locations.items():
        if location_data[1] == "V":
            if i == 0:
                ...
                x = location_id * c1 + c2
            y = (
                50 + (calculate_depth(nodes, location_id)) * 50
            )  # Adjust Y elevation based on node depth
    ...
    for node in nodes:
        ...
        if locations[source_id][1] == "V" and locations[target_id][1] == "V":
            ...
            y1 = level_positions.get(source_id, (0, 0))[
                1
            ]  # Use get() method to provide default value
            x2 = level_positions.get(target_id, (0, 0))[0]
            y2 = level_positions.get(target_id, (0, 0))[
                1
            ]  # Use get() method to provide default value
